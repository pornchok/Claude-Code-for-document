...
- ขาด section บังคับ (บทที่ 1: ขาด 1 ใน 6 mandatory; บทที่ 2+: ขาด 1 ใน 7 mandatory — Analogy เป็น optional ไม่นับในทุกบท)
...
- เนื้อหาหลัก (section 4) มีหลาย approach แต่ไม่ระบุ best practice / popular pattern และไม่ใช้ no-consensus format (หัก 3 ต่อบทที่พบ — ยกเว้น: ถ้าไม่มี official consensus ให้ระบุ use case ของแต่ละ approach ชัดเจนแทนได้ = ผ่าน ไม่หัก)
...
⚠️ **Quote ที่ valid ต้องแสดงให้เห็นชัดว่า checklist item นั้นครบจริง** — quote header เพียงอย่างเดียวโดยไม่มีเนื้อหาที่ตรวจจริง = invalid ตัวอย่าง: checklist "ทุก analogy มี breakdown points" → quote ต้องเป็นบรรทัด "⚠️ ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า: ..." จากไฟล์นั้น ไม่ใช่แค่ quote heading "## 3. Analogy"
...
**ส่วนครบถ้วนของบท (Analogy = optional ไม่นับใน mandatory):**
- บทที่ 1 = 6 mandatory sections: วัตถุประสงค์, Why, เนื้อหาหลัก + self-check ⏸, ตัวอย่าง 3 ระดับ, Common Mistakes + *(source: URL)* inline, Retrieval Questions + เฉลย — ไม่มี Pre-chapter Retrieval, ไม่มี Analogy บังคับ
- บทที่ 2+ = 7 mandatory sections: เดิมทั้ง 6 + Pre-chapter Retrieval ที่อยู่ต้นไฟล์ — ไม่มี Analogy บังคับ
- Analogy เป็น optional ทั้งสองบท — ถ้าใส่: ต้องมี breakdown point 1 ข้อ (mandatory เมื่อมี analogy)
- Pre-chapter Retrieval อยู่ต้นไฟล์ก่อนส่วนที่ 1 ไหม (ไม่ใช่ท้ายบท) — quote บรรทัดแรกของไฟล์นั้นเพื่อยืนยัน

**Analogy check (optional section):**
- ถ้าบทไม่มี analogy เลย → ผ่าน ไม่ต้องตรวจต่อ
- ถ้าบทมี analogy: มี "⚠️ ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า: ..." ระบุ 1 misconception ชัดเจนไหม — ถ้าไม่มีเลย → หัก 5
- breakdown points มีมากกว่า 1 ข้อไหม — ถ้ามี > 1 → หัก 3 (writing standard กำหนดให้ระบุแค่ 1 ข้อที่อันตรายที่สุด — ถ้าระบุหลายข้อจะ dilute impact และ confuse ผู้เรียนมากกว่าช่วย)
...
- อย่างน้อย 1 ⏸ ต่อบทมี Bloom's level ถึง floor ของบทนั้น (บท 1=L1-L2, บท 2-3=L3, บท 4+=L4-L5) ไหม

**Retrieval Questions ท้ายบท (Section 7):**
...
- ไม่ซ้ำกับ misconception ที่ระบุใน Analogy breakdown points แล้ว (ถ้าบทนั้นไม่มี opt-out)
...
**Best practice:**
- เนื้อหาหลัก (section 4) ระบุ best practice / popular pattern ชัดเจน **และ** เหตุผลสั้นๆ ว่าทำไมถึงแนะนำ เมื่อมีหลาย approach ไหม — ไม่ทิ้งให้ผู้เรียนเลือกเองโดยไม่มีข้อมูล (หัก 3 ต่อบทที่พบ — Agent 3 ห้ามหักซ้ำในมิตินี้) ยกเว้น: ถ้า official source ไม่มี consensus → ใช้ no-consensus format (ระบุ use case ของแต่ละ approach ชัดเจน) = ผ่าน ไม่หัก
...
**Analogy check:**
- Analogy มี breakdown points ระบุ specific misconception จริงไหม — ไม่ใช่แค่ "analogy ไม่ครอบคลุม" ต้องบอกว่า "จะเข้าใจผิดว่า [อะไร]" — ถ้าบทใช้ opt-out อย่างถูกต้อง → ผ่าน
  (Agent 1 ตรวจ presence ของ breakdown points; Agent 3 ตรวจ quality — ถ้า Agent 1 หักแล้ว Agent 3 ตรวจเฉพาะ quality ไม่ใช่ presence — ห้ามหัก -5 ซ้ำ ถ้า quality issue ที่ Agent 1 ไม่ได้หักใช้ -3)
...
- Section 7 (Retrieval Questions) มี instruction ครบ 3 elements ของ Generation effect ไหม: (1) หยุดคิดอย่างน้อย 30 วินาที, (2) เขียนคำตอบก่อนดูเฉลย, (3) เขียนเหตุผลสั้นๆ ว่าทำไมถึงตอบแบบนั้น — ถ้าขาด element ใดก็ตาม → หัก 3 คะแนน (ถ้า Agent 1 หักแล้ว → ห้ามหักซ้ำ)
...
**Exercises:**
- Intermediate ใช้ต่าง domain จริงไหม — ให้ list domain ของตัวอย่างในบท (ส่วนที่ 5) ก่อน แล้วเทียบว่า Intermediate exercise ต่างกันอย่างชัดเจน (ไม่ใช่แค่เปลี่ยนชื่อตัวแปรหรือตัวเลข)
- Advanced มี: semi-blind far transfer + Feynman explanation task (ครบ 4 ขั้น: อธิบาย→ระบุ gap→กลับทบทวน→simplify+analogy — ขั้นที่ 4 ต้องมี breakdown points ของ analogy ที่สร้างขึ้น) + elaborative interrogation ไหม
- Self-check ⏸ มีอย่างน้อย 1 ข้อต่อบทที่ถึง Bloom's floor (floor = minimum level; บท 1=L1-L2, บท 2-3=L3, บท 4+=L4-L5) ไหม — floor เป็น minimum ไม่ใช่ ceiling หรือ target ของทุกข้อ
- มี Mixed Review ไหม + มี note อธิบาย interleaving cost ไหม
- ไม่มีข้อ fill-in-the-blank (ทุก level)
- ไม่มีข้อ copy context/ตัวเลขจาก section ตัวอย่าง (ส่วนที่ 5) ไหม
...
→ Launch **Synthesis Agent** ด้วย Task tool ให้อ่าน feedback ทั้งหมด + เอกสารจริง แล้วหา consensus ว่า version ไหนถูกต้องกว่าตาม SKILL goals — ถ้า Synthesis Agent ไม่สามารถ conclude ได้ → escalate user

**กรณี 3 agents ขัดแย้งกันทั้งหมด** (ทั้ง 3 agents ให้ verdict ต่างกันสำหรับ item เดียวกัน):
→ escalate user ทันที ห้ามตัดสินใจเองหรือ launch Synthesis — แสดง 3 positions + tradeoff ให้ user ตัดสิน
...
1. ถ้ามีไฟล์ source-notes.md ชั่วคราว ลบออก
