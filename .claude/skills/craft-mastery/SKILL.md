...
```
รอ user confirm ก่อน proceed STEP 1 — ห้ามข้าม gate นี้
...
ถ้า WebFetch ส่งคืน error, redirect ไปยัง login/paywall, 404, หรือเนื้อหาไม่ใช่ documentation จริง → บันทึก FETCH_FAILED สำหรับ URL นั้นไว้ก่อน อย่า proceed ต่อโดยสมมติว่า fetch สำเร็จ — ดำเนิน fetch ต่อสำหรับ URL ที่เหลือก่อน แล้วรวบรวม FETCH_FAILED ทั้งหมดและ handle ใน STEP 1.4 ครั้งเดียวหลังจาก fetch ครบ URL ที่ต้องการในรอบนี้
...
- **บทที่ 2+**: sections 1,2,4,5,6,7,8 บังคับ (7 mandatory — ไม่มี Analogy บังคับ แต่ต้องมี Pre-chapter Retrieval)
...
3. Analogy (optional — default คือข้ามส่วนนี้)
   ⚠️ Default: ข้ามส่วนนี้เลย ถ้าต้องนึกนานว่าจะเปรียบกับอะไร = สัญญาณว่าไม่มี analogy ที่ดีพอ
   analogy ที่งงคือแย่กว่าไม่มี — ห้าม force
   ⚠️ ถ้าตัดสินใจใส่ Analogy: breakdown point (บรรทัด "⚠️ ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า: ...") เป็น mandatory — ขาดแล้วถือว่า fail checklist

   ใส่เฉพาะถ้าผ่านทั้ง 3 ด่านนี้ในทันที:

   ด่าน 1 — Relational fit: วาด A→B ใน analogy และ X→Y ใน concept — ประเภทความสัมพันธ์เหมือนกันไหม?
   - ✅ pass: queue (FIFO) ↔ คิวรอรถเมล์ — ordering-by-arrival-time เหมือนกัน
   - ❌ fail: thread pool ↔ ทีมงาน — หน้าตาเหมือนแต่ scheduling/reuse mechanism ต่างกัน

   ด่าน 2 — Audience familiarity: source domain ต้องเป็นสิ่งที่ผู้เรียนรู้จักทันทีโดยไม่ต้องอธิบาย
   - default audience: "urban adult ที่ใช้ smartphone ในชีวิตประจำวัน"

   ด่าน 3 — Genuine simplification: ถามตัวเองว่า "analogy นี้ทำให้เข้าใจง่ายขึ้นจริง หรือแค่ฟังดูคล้าย?"
   - ถ้าไม่แน่ใจ → ข้ามไปส่วน 4

   ถ้าผ่านทั้ง 3 ด่าน → เขียน analogy พร้อม:
   "⚠️ ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า: [1 misconception ที่อันตรายที่สุด]"
   ระบุแค่ 1 ข้อ — ถ้าระบุหลายข้อจะ confuse ผู้เรียนมากกว่าช่วย

4. เนื้อหาหลัก — จาก QUOTE ใน source-notes ภาษาไทย ศัพท์เทคนิค/code เป็น English
...
6. Common Mistakes — ก่อนเขียน mistake แต่ละข้อ: ตรวจว่ามี QUOTE ใน source-notes รองรับไหม
   ...
   - ห้าม mistake ซ้ำกับ misconception ที่ระบุใน analogy breakdown points แล้ว — ต้องเป็น mistake ใหม่ (ถ้าส่วนที่ 3 ใช้ opt-out ไม่มี analogy → ข้อห้ามนี้ไม่ apply เขียน mistake ได้อิสระตาม QUOTE)
...
7. สรุปบท — คำถาม Retrieval 2-3 ข้อ (ผู้เรียนตอบก่อน แล้วค่อยดูเฉลยด้านล่าง — ห้ามเป็นแค่ bullet list)
...
8. Pre-chapter Retrieval (บทที่ 2+ เท่านั้น) — วางไว้เป็น **section แรกสุดของไฟล์ ก่อนส่วนที่ 1** ไม่ใช่ท้ายบท
...
- Intermediate: Application in novel context — สถานการณ์ที่ไม่มีใน section ตัวอย่าง (ส่วนที่ 5) ของบทนั้น → novel context = near transfer — เปลี่ยน use-case หรือ problem scenario โดยยังอยู่ใน application domain ที่ผู้เรียนคุ้นเคย ไม่ใช่แค่เปลี่ยนชื่อตัวแปรหรือตัวเลข (เช่น ถ้าตัวอย่างในบทใช้ e-commerce → exercise ใช้ healthcare หรือ logistics ได้ เพราะทั้งหมดเป็น business application domain) — ไม่ใช่ far transfer ข้ามไปชีวิตประจำวันหรืออธิบายให้คนทั่วไปเข้าใจ (นั่นคือ Advanced level)
- Advanced: Synthesis / Diagnosis — หา bug, ออกแบบระบบ, เปรียบเทียบ tradeoff
  ...
  ต้องมีอย่างน้อย 1 ข้อ explanation/metacognitive task (ไม่ใช่ far transfer — เป็น teaching task ที่ใช้ Feynman Technique ครบ 4 ขั้น): "อธิบาย concept นี้ให้คนที่ไม่รู้ [topic domain] เข้าใจ โดย: (1) อธิบายราวกับสอนเด็ก ใช้ภาษาง่ายที่สุด, (2) ระบุจุดที่คุณอธิบายไม่ได้หรือสะดุด — ส่วนนั้นคือ gap ของความเข้าใจ, (3) กลับทบทวนส่วนนั้นในเอกสารก่อนอธิบายใหม่, (4) simplify ด้วย analogy จากชีวิตประจำวัน + ระบุ breakdown points ของ analogy ที่คุณสร้างขึ้น"
  ⚠️ (Claude note — ห้ามนำไปใส่ใน exercises.md): ขั้นที่ 2 และ 3 สำคัญมาก เป็น mechanism ที่ทำให้ Feynman Technique work จริง — ขั้นที่ 4 ส่วน "ระบุ breakdown points" เป็น addition เพื่อป้องกัน misleading analogy ไม่ใช่ส่วนของ Feynman Technique ดั้งเดิม
...
⚠️ ตรวจสอบ novel context สำหรับ exercises.md: list domain ของตัวอย่างในบท (ส่วนที่ 5) แล้ว verify ว่า Intermediate exercise ใช้ domain ที่ต่างกันอย่างชัดเจน (ทำก่อน STEP 5 สำหรับ exercises) — ถ้า domain ยังเหมือนกัน → ต้องแก้ exercise ให้ต่าง domain ก่อน ห้าม proceed STEP 5 โดยไม่แก้
...
2. ถ้าผ่าน → Read ไฟล์เอกสารใหม่เพื่อยืนยันว่า code ในเอกสารตรงกับที่ทดสอบจริง → insert พร้อม: ระบุภาษา + output จริง + version ที่ทดสอบ
...
⚠️ **Blocking gate**: ถ้า checklist item ใด fail → ต้องแก้ไขให้ผ่านก่อน proceed STEP 6 ห้าม proceed โดยมี fail อยู่ (เช่น ถ้า Intermediate domain เหมือนตัวอย่างในบท → กลับแก้ก่อน)

- [ ] ทุกบทมี mandatory sections ครบ (บทที่ 1: sections 1,2,4,5,6,7 — 6 mandatory; บทที่ 2+: sections 1,2,4,5,6,7,8 — 7 mandatory; section 3 Analogy เป็น optional ไม่นับ) → verified in: [list]
- [ ] เนื้อหาหลัก (section 4) ระบุ best practice / popular approach ชัดเจนเมื่อมีหลาย approach — ไม่ทิ้งให้ผู้เรียนเลือกเองโดยไม่มีข้อมูล (ยกเว้น: ถ้า official source ไม่มี consensus → ใช้ no-consensus format ระบุ use case ของแต่ละ approach ชัดเจน = ผ่าน) → verified in: [list]
...
- [ ] ถ้ามี Analogy: มี "⚠️ ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า: ..." ระบุ 1 misconception ชัดเจน → verified in: [list] (ถ้าไม่มี Analogy section เลย = ผ่าน ไม่ต้องตรวจ)
...
- [ ] glossary ครอบคลุมทุกคำที่ introduce ครั้งแรก + มี SOURCE URL กำกับทุกคำ → verified in: glossary.md
...
- [ ] Common Mistakes ไม่ซ้ำกับ misconception ที่ระบุใน Analogy breakdown points แล้ว (ถ้าบทนั้นไม่มี Analogy section เลย → ผ่าน ไม่ต้องตรวจ) → verified in: [list]
- [ ] Self-check ⏸ ทุกบทมีอย่างน้อย 1 ข้อที่ถึง Bloom's floor ของบทนั้น (บท 1=L1-L2, บท 2-3=L3, บท 4+=L4-L5) → verified in: [list]
...
- [ ] Advanced exercises มี Feynman explanation task ครบ 4 ขั้น (อธิบาย→ระบุ gap→กลับทบทวน→simplify+analogy) รวมถึง breakdown points ของ analogy ที่สร้างขึ้น → verified in: exercises.md
...
หัก 5 คะแนนต่อรายการ (นับต่อ instance — ถ้า violation เดียวกันเกิดในหลายบท เช่น ขาด breakdown points ใน 3 บท = หัก 3×5 = 15 คะแนน ไม่ใช่ 5 คะแนน):
- ข้อมูลที่ verify ไม่ได้หรืออาจผิด
- code ที่ไม่ได้ทดสอบโดยไม่มี label
- ขาด section บังคับ (บทที่ 1: 1 ใน 6 mandatory; บทที่ 2+: 1 ใน 7 mandatory — Analogy เป็น optional ไม่นับ)
- analogy ที่ใส่ไว้แล้วทำให้เข้าใจผิดอย่างชัดเจน หรือมี analogy แต่ไม่มี breakdown point เลย (ถ้าไม่มี analogy section เลย = ไม่หักคะแนน)
...
- Self-check ⏸ ไม่มีเฉลยสั้น หรือไม่มี remediation direction (ทั้งสองเป็น mandatory ตาม STEP 3 ส่วนที่ 4)
...
- ทุกบทมีครบ mandatory sections ไหม: บทที่ 1 = 6 mandatory (วัตถุประสงค์, Why, เนื้อหาหลัก, ตัวอย่าง 3 ระดับ, Common Mistakes, Retrieval Questions — ไม่มี Analogy บังคับ, ไม่มี Pre-chapter Retrieval), บทที่ 2+ = 7 mandatory (เดิม + Pre-chapter Retrieval) — Analogy เป็น optional ทั้งสองบท; ถ้าใส่ Analogy ต้องมี '⚠️ ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า: ...' 1 ข้อ (mandatory ถ้ามี analogy)
...
- exercises มีครบ 3 ระดับ cognitive + Mixed Review + เฉลยไหม — Intermediate ใช้ต่าง domain (ไม่ใช่แค่เปลี่ยนชื่อตัวแปร) + Mixed Review มีอย่างน้อย 1 ข้อ discrimination task + ทุกกลุ่ม exercises มี Timing instructions ("ทำหลังอ่านบทที่ X") + ไม่มี fill-in-the-blank ทุก level + ไม่ copy context/ตัวเลขจากตัวอย่างในบท (ทุก level) + Advanced มี Feynman task ครบ 4 ขั้น (อธิบาย→ระบุ gap→กลับทบทวน→simplify+analogy+breakdown points)
- glossary ครอบคลุมคำที่ introduce ครั้งแรก + มี SOURCE URL ไหม
...
อ่านเอกสารทั้งหมดในฐานะ practitioner ที่ใช้ $TOPIC ใน production จริง ตรวจ:
...
- Exercises: Intermediate ใช้ต่าง domain จริงไหม, Advanced มี cross-domain ไหม, มี Mixed Review ไหม
- exercises ไม่มีข้อที่เป็น fill-in-the-blank ไหม (ทุก level)
- exercises ไม่มีข้อที่ copy context หรือตัวเลขโดยตรงจาก section ตัวอย่าง (ส่วนที่ 5) ไหม (ทุก level)
- แต่ละบทมีไม่เกิน 3-5 concept ไหม
- ภาษาไทยเป็นธรรมชาติไหม ย่อหน้าสั้นพอไหม (ไม่เกิน 4-5 บรรทัด)
- Learner journey simulation: อ่าน series ทั้งหมดในฐานะผู้เรียนที่มี prerequisites ตาม STEP 0 พยายามระบุจุดที่จะรู้สึก "งง" หรือ "ข้ามขั้น" (target: 3 จุด — ถ้าหาได้น้อยกว่า 3 แสดงว่าเอกสารดีมาก ไม่ต้องบังคับ) และยืนยันว่าทุกจุดที่พบได้รับการจัดการในเอกสารแล้ว (หรือระบุว่าไม่ได้จัดการ → หักคะแนน)
ให้คะแนน 0-100 ตาม rubric + bullet list จุดแก้ไขพร้อมระบุไฟล์
...
Launch **Dispute Arbiter Agent** (ผ่าน Task tool) ด้วย (a) feedback ของ review agent, (b) ข้อโต้แย้ง + เหตุผล, (c) quote ส่วนที่พิพาทจากเอกสาร → arbiter ให้ verdict → ถ้า sides with reviewer: แก้ไข; ถ้า sides with main: waive; ถ้า "unclear" หรือ main agent ยังไม่เห็นด้วยหลัง verdict → main agent ตัดสินใจเอง: ใช้ review agent's feedback เป็น default (conservative) + บันทึก "[Autonomously resolved: deferred to reviewer feedback]" ใน report ห้าม launch Arbiter ซ้ำ

**กรณี feedback จาก 2+ agents ขัดแย้งกัน** (score ต่างกัน > 10 คะแนนสำหรับ item เดียวกัน หรือ explicit contradiction):
→ Launch **Synthesis Agent** (ผ่าน Task tool) ให้อ่าน feedback ทั้งหมด + เอกสารจริง แล้วหา consensus — ถ้า Synthesis Agent conclude ไม่ได้ → escalate user

**ถ้า 3 agents ขัดแย้งกันหมด:** แสดง 3 positions + tradeoff แล้ว escalate user ทันที ห้ามตัดสินใจเอง

⚠️ Dispute cap: เปิด Dispute Arbiter ได้ไม่เกิน 3 ข้อต่อรอบ review — ถ้าต้อง dispute เกิน 3 ข้อ → escalate user แทน เพราะแปลว่ามี systematic disagreement ที่ต้องการ human judgment
