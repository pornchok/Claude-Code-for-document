...
- **ออกแบบ** JSR223 Assertion (Groovy) สำหรับ validation ที่ซับซ้อนเกินกว่า built-in assertions จะทำได้ และอธิบาย trade-off ของวิธีนี้
...
### ระดับ Advanced: JSR223 Assertion (Groovy) สำหรับ Complex Validation

สถานการณ์: ระบบ inventory management มี API `GET /inventory/batch` ที่ return รายการสินค้า ต้องการ validate ว่า:
1. รายการทั้งหมดที่ return มี `quantity >= 0` (ไม่มีค่าติดลบ)
2. รายการที่มี `status: "active"` ต้องมี `price > 0`
3. จำนวน items รวมต้องไม่เกิน 100 รายการ (pagination guard)

Built-in assertions ทำไม่ได้ครบทั้ง 3 ข้อในคราวเดียว — ต้องใช้ **JSR223 Assertion** กับ Groovy script

**วิธีเพิ่ม:** คลิกขวา → Add → Assertions → JSR223 Assertion
...
```groovy
// ยังไม่ได้ทดสอบด้วย JMeter จริง — ต้องการ JMeter 5.6.3
// Validate inventory batch response

import groovy.json.JsonSlurper

// Parse response body เป็น JSON object
def response = new JsonSlurper().parseText(prev.getResponseDataAsString())
def items = response.items

// Validation 1: ตรวจจำนวน items ไม่เกิน 100
if (items.size() > 100) {
    AssertionResult.setFailure(true)
    AssertionResult.setFailureMessage("Too many items returned: ${items.size()} (max 100)")
    return
}

// Validation 2: ทุก item ต้องมี quantity >= 0
def negativeQty = items.findAll { it.quantity < 0 }
if (negativeQty) {
    def ids = negativeQty.collect { it.id }.join(', ')
    AssertionResult.setFailure(true)
    AssertionResult.setFailureMessage("Items with negative quantity found: [${ids}]")
    return
}

// Validation 3: active items ต้องมี price > 0
def activeWithNoPrice = items.findAll { it.status == "active" && it.price <= 0 }
if (activeWithNoPrice) {
    def ids = activeWithNoPrice.collect { it.id }.join(', ')
    AssertionResult.setFailure(true)
    AssertionResult.setFailureMessage("Active items with invalid price (must be > 0): [${ids}]")
    return
}

// ผ่านทุก validation
AssertionResult.setFailure(false)
log.info("Inventory validation passed: ${items.size()} items")
```

**Trade-off ที่ต้องรู้ก่อนใช้ JSR223 Assertion:**

| ข้อดี | ข้อเสีย |
|-------|---------|
| Validate ได้ทุก logic ที่ซับซ้อน | เพิ่ม CPU overhead ต่อ request |
| Debug ได้ง่ายผ่าน failure message | Script ต้องมี test ของตัวเอง — ถ้า script มี bug จะ false positive |
| Reuse Groovy library ที่มีอยู่ได้ | Maintenance สูงกว่า built-in assertions |
| — | ถ้า Groovy script throw exception JMeter อาจ mark เป็น pass (ต้อง handle exception เสมอ) |
...
Groovy implement Compilable interface — JMeter จะ compile script ครั้งแรกแล้ว cache ไว้ ทำให้ iteration ถัดไปเร็วกว่า BeanShell มาก แต่ยังช้ากว่า built-in assertions อยู่ดี

**แนวทาง:** ใช้ JSR223 Assertion เฉพาะเมื่อ built-in assertions จริงๆ ทำไม่ได้ ไม่ใช่ใช้แทนทุกอย่าง
