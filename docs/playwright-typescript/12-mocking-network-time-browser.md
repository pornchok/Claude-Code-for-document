...
`page.routeWebSocket()` เพิ่มใน v1.48 สำหรับ intercept WebSocket connections — WebSocket คือ protocol สำหรับ real-time communication แบบ bidirectional (เช่น live chat, notification) ต่างจาก HTTP ที่ client ต้อง request ทุกครั้ง demo app ของเราใช้ WebSocket สำหรับ broadcast todo updates:
...
test('ไม่แสดงสินค้าเมื่อ API ล้มเหลว', async ({ page }) => {
  await page.route('**/api/products*', async route => {
    await route.fulfill({
      status: 500,
  ...
  // เมื่อ API return 500, shop.html throw unhandled error และ product grid ว่างเปล่า
  // no-results element ไม่ถูก set visible เพราะ renderProducts() ไม่ถูกเรียก
  await expect(page.locator('[data-testid="product-grid"] .card')).toHaveCount(0);
  // heading ยังคงแสดงให้รู้ว่า page ไม่ crash
  await expect(page.getByRole('heading', { name: 'Product Shop' })).toBeVisible();
});
```
...
test('network failure: product grid ว่างเมื่อ API abort', async ({ page }) => {
  ...
  // เมื่อ network abort, loadProducts() throw unhandled error
  // no-results ไม่ถูก set visible — product grid ว่างเปล่า
  await expect(page.locator('[data-testid="product-grid"] .card')).toHaveCount(0);
  // heading ยังแสดงอยู่ — page ไม่ crash
  await expect(page.getByRole('heading', { name: 'Product Shop' })).toBeVisible();
});
```
