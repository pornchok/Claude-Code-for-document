...
**Mocking เหมือนการใช้ stunt double แทนนักแสดงหลักในฉาก action** — ถ่าย fight scene นักแสดงจริงไม่ต้องเสี่ยงอันตราย ส่ง stunt double เข้าแทน ผลลัพธ์บนจอเหมือนกัน ถ่ายซ้ำได้ไม่จำกัดรอบ และกำกับได้ว่าอยากให้ฉากออกมาอย่างไร

ใน E2E test: "นักแสดงจริง" คือ external API จริง "stunt double" คือ mock response ที่เรา control ได้ 100%

⚠️ ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า:

- **Mock test เชื่อถือได้เท่ากับ real integration test** — จริงๆ ถ้า real API เปลี่ยน response format mock จะไม่รู้ตัว tests ยังคง pass ต่อแม้ production จะ broken ดังนั้นต้องมี balance ระหว่าง mock tests สำหรับ speed/isolation และ real integration tests สำหรับ contract verification
- **ควร mock ทุก API call เสมอ** — mock เฉพาะ dependency ที่ต้องการ isolate จริงๆ เช่น external payment gateway, third-party analytics ไม่ควร mock application API ของตัวเองในทุก test เพราะจะ miss bugs ใน backend logic
...
test.describe('Dashboard — Clock API ควบคุมวันที่แสดง', () => {
  ...
  test('แสดงวันที่ถูกต้องเมื่อ Clock fix เป็นวันที่ 2030-07-15', async ({ page }) => {
    ...
    // demo app มี date picker ที่ /components — ทดสอบว่า input-date ทำงานกับ fixed clock
    await page.goto('http://localhost:3000/components');
    await page.fill('[data-testid="input-date"]', '2030-07-15');
    await expect(page.locator('[data-testid="date-result"]')).toContainText('2030-07-15');
  });
