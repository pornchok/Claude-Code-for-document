...
#### 2. `page.getByLabel('text')` — form inputs
...
#### 3. `page.getByPlaceholder('text')` — placeholder ใน input
...
test('shows locator priority order in practice', async ({ page }) => {
  ...
  // ลำดับที่ 2: getByLabel — สำหรับ form inputs
  ...
  // ลำดับที่ 4: getByText — ตรวจ message หลัง login
...
// สถานการณ์ที่ต้องใช้ or() — UI แสดง element ต่างกันบน mobile/desktop
test('finds action button regardless of viewport', async ({ page }) => {
  await page.goto('http://localhost:3000/shop');

  // Desktop: ปุ่ม "Add to cart" — Mobile: icon button ที่มี aria-label เดียวกัน
  const addToCartAction = page
    .getByRole('button', { name: 'Add to cart' })
    .or(page.getByRole('button', { name: 'Add to cart', exact: true }));

  // ใช้ filter ร่วมกับ or เพื่อหาตัวที่ visible จริงๆ
  const visibleAction = addToCartAction.filter({ visible: true });
...
| ไม่ sure ว่า element ไหน visible | หลาย element match แต่บางตัวซ่อนอยู่ | ใช้ `filter({ visible: true })` |
...
- **Priority order** ของ locator ตาม Playwright: getByRole → getByLabel → getByPlaceholder → getByText → getByAltText → getByTitle → getByTestId → locator(css) — เรียงจาก semantic มากไปน้อย
...
**คำถามที่ 1:** ใช้ `page.getByPlaceholder('Search...')` เพราะนั่นคือสิ่งเดียวที่ระบุตัวตนของ input ได้ตอนนี้ — เมื่อ designer เพิ่ม label ควรเปลี่ยนเป็น `page.getByLabel('Search products')` ทันที เพราะ label อยู่ในลำดับ priority สูงกว่า (ลำดับ 2 vs ลำดับ 3) และมั่นคงกว่าเพราะ label คือสิ่งที่ user อ่านเห็น
