...
**CI pipeline คือโรงงานที่ทำทุกอย่างอัตโนมัติ — เมื่อ engineer กด push โรงงานเริ่มทำงานทันทีโดยไม่ต้องสั่ง**

ลองนึกถึงโรงงานประกอบรถที่มี assembly line: ทันทีที่ชิ้นส่วนใหม่เข้ามา (push code), สายพานเริ่มเคลื่อน — station แรก checkout code, station ที่สอง install dependencies, station ที่สาม รัน tests, station ที่สี่ export report ถ้าชิ้นส่วนผ่านทุก station, รถออกจากโรงงานได้ (merge) ถ้าไม่ผ่าน, สายพานหยุดและไฟแดงติด (CI fail) — engineer ต้องแก้ก่อนจึงจะเดินต่อได้

Shard เปรียบได้กับการมีหลาย assembly line ทำงานพร้อมกัน — แทนที่จะใช้ line เดียวทำทีละชิ้นจนครบ, แบ่งงานออกเป็น 4 ส่วนให้ 4 lines ทำพร้อมกัน เร็วขึ้น 4 เท่า

⚠️ ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า:

- **CI แก้ flaky tests ให้อัตโนมัติ** — CI แค่รัน tests ที่มีอยู่ ถ้า test ของคุณ flaky (บางครั้งผ่าน บางครั้งไม่ผ่านโดยไม่มี logic เปลี่ยน) CI ก็จะ fail ซ้ำ — ต้องแก้ test ให้ stable ก่อน CI จะช่วยได้
- **`CI=true` เปลี่ยน test behavior ทันทีโดยอัตโนมัติ** — `CI=true` เป็นแค่ environment variable ธรรมดา behavior เปลี่ยนได้ก็ต่อเมื่อ `playwright.config.ts` ของคุณอ่านค่านี้และตั้งค่าต่างออกไป เช่น `retries: process.env.CI ? 2 : 0` — ถ้า config ไม่ได้อ่าน `CI`, setting ก็ไม่เปลี่ยน

---

## 4. เนื้อหาหลัก

### 4.1 GitHub Actions — Complete Workflow
...
### 4.2 ผลของ `CI=true` ใน playwright.config.ts
...
### 4.3 GitHub Reporter (`--reporter=github`)
...
### 4.4 Docker — Official Playwright Image
...
### 4.5 Sharding — รัน Tests Parallel ใน CI
...
### 4.6 `--only-changed` — รัน Selective Tests
