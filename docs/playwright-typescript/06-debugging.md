...
- เลือก strategy ที่ถูกต้องสำหรับ debug element ที่ปรากฏชั่วคราว เช่น loading spinner หรือ toast notification
...
### 3.2 page.pause() — Breakpoint ใน Code
...
### 3.4 Headed Mode — ดู Browser ทำงาน
...
### 3.5 Screenshot และ Video — หลักฐานอัตโนมัติ
...
### 3.6 VS Code Extension — Debug จาก Editor
...
### 3.7 Codegen — บันทึก Action แล้วสร้าง Test
...
### 3.8 UI Mode — Interactive Test Runner พร้อม Watch
...
### 3.11 Debugging Transient Elements — Loading, Toast, Animation

element บางประเภทขึ้นมาแค่ชั่วคราวแล้วหายไป เช่น loading spinner ระหว่างรอ API, toast notification ที่หายใน 3 วินาที, skeleton loader ก่อน content โหลดเสร็จ — ทำให้ codegen หรือ Inspector ตามไม่ทัน

มีหลายวิธีรับมือกับ element ประเภทนี้ขึ้นอยู่กับสถานการณ์:

---

**วิธีที่ 1 — Trace Viewer (แม่นที่สุด ไม่พลาดแม้แต่ frame เดียว)**

Trace Viewer บันทึกทุก DOM change ทุก millisecond ตลอดการ run ไม่ว่า element จะโผล่นานแค่ไหน เป็นวิธีที่เชื่อถือได้มากที่สุดสำหรับ element ที่หายเร็ว

```typescript
// playwright.config.ts — เปิดชั่วคราวระหว่าง debug เท่านั้น
export default defineConfig({
  use: {
    trace: 'on',
  },
...
เปิด Trace Viewer → ดู Timeline → เลื่อนไปที่ช่วงหลัง action ที่ trigger loading → จะเห็น DOM snapshot ที่มี loading element อยู่ → กด "Pick locator" ใน snapshot นั้นได้เลย *(source: https://playwright.dev/docs/trace-viewer — "DOM snapshots allow you to fully interact with the DOM during debugging")*

**คืนค่าหลัง debug เสร็จ:** เปลี่ยน `trace: 'on'` กลับเป็น `trace: 'on-first-retry'` เพื่อไม่ให้กิน disk

---

**วิธีที่ 2 — Network Throttle ใน DevTools (เร็วสำหรับหา locator)**

เปิด headed mode แล้ว throttle network ให้ช้าลงเพื่อให้ element อยู่นานพอที่จะ inspect ได้

```bash
npx playwright test --headed
```

เมื่อ browser เปิดขึ้นมา → กด F12 → Network tab → เปลี่ยนจาก "No throttling" เป็น **Slow 3G** → trigger action ใหม่ → loading element จะช้าพอให้ไป Elements panel แล้ว inspect locator ได้

**ข้อจำกัด:** ใช้ได้กับ Chromium เท่านั้น และต้องการ headed mode

---

**วิธีที่ 3 — page.pause() หลัง trigger**

```typescript
// partial example
test('find loading locator', async ({ page }) => {
  await page.goto('http://localhost:3000/shop');
  await page.getByRole('button', { name: 'Search' }).click(); // trigger loading
  await page.pause(); // หยุดทันที — ถ้า loading ยังอยู่จะ inspect ได้ใน Inspector
});
```

**ข้อจำกัด:** ถ้า loading หายเร็วกว่าที่ JavaScript จะ reach บรรทัด `page.pause()` จะจับไม่ทัน ในกรณีนั้นให้ใช้ Trace Viewer แทน *(source: https://playwright.dev/docs/debug)*

---

**วิธีที่ 4 — Break on DOM Mutations ใน DevTools**

เปิด headed mode → DevTools → Elements panel → right-click ที่ parent element ของบริเวณที่ loading น่าจะโผล่ → **"Break on" → "subtree modifications"** → browser จะ pause execution ทันทีที่มีการเพิ่ม/ลบ/แก้ element ใต้ node นั้น → inspect ได้ที่จุด pause

**ข้อกำหนด:** ต้องตั้ง breakpoint ก่อน trigger action และต้องการ headed mode

---

**วิธีที่ 5 — ดู Source Code โดยตรง (เร็วที่สุดถ้าเป็น app ของตัวเอง)**

ค้นหาคำว่า `loading`, `spinner`, `toast`, `skeleton` ใน frontend codebase โดยตรง — component เหล่านี้มักมี `data-testid` หรือ class ที่ตั้งชื่อสื่อความหมายอยู่แล้ว ไม่ต้องไล่จับ runtime เลย

---

**เปรียบเทียบวิธีตาม use case:**

| Use Case | วิธีที่เหมาะ |
|----------|------------|
| element เร็วมาก < 100ms | Trace Viewer |
| มี access source code | ดู source โดยตรง |
| ต้องการ inspect แบบ interactive | Network throttle + headed mode |
| รู้ว่า loading น่าจะอยู่นานพอ | `page.pause()` หลัง trigger |
| ไม่รู้ว่า loading โผล่ตรงไหนของ DOM | Break on DOM mutations |

---

### 3.12 Decision Guide — เลือก Tool ให้ถูก
...
| Element ปรากฏชั่วคราว (loading, toast) | Trace Viewer (`trace: 'on'` ชั่วคราว) หรือ Network throttle |

---

### 3.13 RF/Selenium vs Playwright — Debug Workflow
...
- **Transient Elements** — element ที่ปรากฏชั่วคราวใช้ Trace Viewer (`trace: 'on'`) เป็น default เพราะบันทึกครบทุก frame, Network throttle สำหรับ inspect แบบ interactive, `page.pause()` ถ้า element อยู่นานพอ
- **Decision guide** — เลือก tool ให้ตรงกับสถานการณ์ (local dev / CI failure / เขียน test ใหม่ / transient element)
