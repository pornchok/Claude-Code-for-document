...
**โปรเจค Playwright เหมือนสถานีดับเพลิง**

สถานีดับเพลิงที่ดีแยกพื้นที่ชัดเจน: ห้องรถดับเพลิง (UI tools), ห้องอุปกรณ์ (API tools), ห้อง briefing (fixtures/setup), คลังข้อมูล (types/data) — ทุกคนรู้ว่าของอยู่ที่ไหน แต่ไม่ได้แยกห้องย่อยสำหรับ "อุปกรณ์สีแดง" vs "อุปกรณ์สีน้ำเงิน" เพราะมันไม่จำเป็น

⚠️ **ถ้าเชื่อ analogy นี้ 100% จะเข้าใจผิดว่า:**
- โปรเจคต้องมีห้องแยกสำหรับทุก feature เหมือนสถานีใหญ่ — ความจริง: project ขนาดกลาง (< 200 tests) มี `tests/web/` กับ `tests/api/` แค่ 2 ห้องก็พอ
- ทุก utility ต้องอยู่คนละห้อง — ความจริง: ถ้า helper ไม่ถูกใช้ซ้ำจาก 2+ ที่ ไม่ต้องแยก folder

---

## 4. โครงสร้างที่แนะนำ

### 4.1 The Recommended Structure
...
### 4.3 Page Objects — เฉพาะ UI Abstraction
...
### 4.4 Fixtures — Shared Setup ที่ใช้ได้กับทั้ง Web และ API
...
### 4.5 Helpers — Pure Functions ที่ Test ได้
...
### 4.6 Playwright Config ที่ support Web + API
