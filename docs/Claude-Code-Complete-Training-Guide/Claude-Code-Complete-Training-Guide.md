...
> - URLs และคำสั่งต่างๆ อาจเปลี่ยนแปลงได้ กรุณาตรวจสอบจาก [เอกสารอย่างเป็นทางการของ Claude](https://code.claude.com/docs)
...
> **เลือก 1 วิธีตามที่คุณมี:**

**วิธีที่ 1 (แนะนำ): มี Claude subscription แล้ว (Pro/Max)**
- ถ้ามี Claude.ai account (Pro หรือ Max) อยู่แล้ว → ไม่ต้องทำอะไรเพิ่ม!
- ตอนรัน `claude` ครั้งแรก จะเปิด browser ให้ login ด้วย Claude.ai account อัตโนมัติ

**วิธีที่ 2: ใช้ API Key (สำหรับองค์กร/ผู้ใช้ API billing)**
...
5. ต้องเติมเงิน: Settings → Billing → Add Credit
...
| `Esc` | หยุดการตอบกลับกลางคัน (งานที่ทำไปแล้วยังอยู่ — redirect ใหม่ได้เลย) |
...
- [ ] Login: ถ้ามี Claude Pro/Max → เลือก "Sign in with Claude.ai" ในหน้า browser ที่เปิดขึ้นมา; ถ้าใช้ API key → วาง key ที่ prompt
...
║  🔴 ปุ่มฉุกเฉิน                                                    ║
║  ─────────────                                                    ║
║  Esc                     หยุดตอบกลับ (งานเก่ายังอยู่, redirect ได้) ║
║  Ctrl+C                  ยกเลิกคำสั่ง                              ║
║                                                                   ║
║  ⚠️ ข้อควรระวัง                                                    ║
║  ──────────────                                                   ║
...
### สิ่งที่ Claude Code ทำไม่ได้ (โดย default)

```
❌ ทำงานเองโดยไม่ถาม (default ต้องขออนุญาตทุกครั้ง)
❌ Push code หรือ deploy โดยไม่บอก
❌ ลบไฟล์สำคัญโดยไม่ยืนยัน
```

> **💡 หมายเหตุสำหรับมือใหม่:**
> - **Internet**: Claude Code มี WebFetch tool ให้ดึงข้อมูลจาก URL ได้ แต่ไม่ได้ browse อย่างอิสระ
> - **จำข้ามวัน**: มีทั้ง CLAUDE.md (instructions ถาวร) และ Auto Memory (จำสิ่งที่เรียนรู้อัตโนมัติ) และ `claude -c` (resume session เดิม) — Claude Code จำได้ข้ามวันถ้าตั้งค่าให้จำ
...
| **Windows** | Windows 10 1809+ หรือ Windows Server 2019+ (WSL2 ไม่จำเป็น — native Windows ทำงานได้) |
| **Hardware** | RAM 4 GB+, x64 หรือ ARM64 |
| **Node.js** | v18+ (เฉพาะ npm install เท่านั้น — native installer ไม่ต้องการ) |
...
claude doctor
# ตรวจสอบการติดตั้งอย่างละเอียด (CLI command ใช้ได้จาก terminal โดยตรง)
# หรือเปิด Claude session แล้วพิมพ์ /doctor ข้างใน session ก็ได้ผลเหมือนกัน
```

> **⚠️ หมายเหตุ:** NPM installation (`npm install -g @anthropic-ai/claude-code`) ยังใช้ได้ แต่ Native Installer เป็นวิธีแนะนำหลัก (auto-update, ติดตั้งง่ายกว่า)

## 2.3 Authentication

มี 2 วิธีใช้งาน — เลือกตามที่คุณมี:

### วิธีที่ 1: Claude Subscription (Pro/Max) — แนะนำสำหรับผู้ใช้ทั่วไป
```bash
# รัน claude ครั้งแรก — จะเปิด browser อัตโนมัติ
claude

# Browser จะขึ้นมาให้ login ด้วย Claude.ai account
# → เลือก "Sign in with Claude.ai"
# → ใส่ email และ password ของ Claude.ai
# ✅ เสร็จ! ไม่ต้อง API key
```

> 💡 **ข้อดี:** ใช้ quota เดิมกับ Claude.ai, ไม่ต้องจัดการ API key

### วิธีที่ 2: API Key — สำหรับองค์กร/นักพัฒนา
```bash
# ตั้งค่า API key (จาก console.anthropic.com)
export ANTHROPIC_API_KEY="sk-ant-..."
claude

# หรือแจ้ง claude auth login --console
claude auth login --console
```

> 💡 **ข้อดี:** จ่ายตาม usage จริง, เหมาะกับ CI/CD และระบบ automation
...
| รัน `/doctor` ใน Claude session หลังติดตั้ง | ข้ามการตรวจสอบไปเลย |
...
**JSON Output Structure** (fields หลักที่ใช้บ่อย):
```json
{
  "type": "result",
  "subtype": "success",
...
| `auto` | Smart auto-approval (AI classifies safety) | Trusted workflows |
...
| `/config` → Editor mode | Toggle vim keybindings (ใช้แทน `/vim` ที่ถูกลบออกใน v2.1.92) |
| `/agents` | Manage subagents |
| `/hooks` | Configure hooks |
| `/mcp` | Manage MCP servers |
| `/plugin` | Manage plugins |
| `/batch <task>` | Parallel operations across codebase |
| `/rewind` | Rewind code/conversation to checkpoint |
| `/btw <question>` | Quick side question (ไม่เพิ่ม context) |
| `/diff` | Interactive diff viewer |
| `/goal <condition>` | Set auto-completion goal |
| `/fast` | Toggle fast mode |
| `/schedule` | Create/manage recurring routines |
...
// .claude/settings.json
{
  ...
  // MCP Server Restrictions (managed settings only)
  "allowedMcpServers": [
    { "serverName": "github" },
...
```bash
# ติดตั้งและเพิ่มเข้า Claude Code ในคำสั่งเดียว (npx จะดาวน์โหลดให้อัตโนมัติ)
...
```bash
# ต้องใช้ GitHub Personal Access Token (PAT)
claude mcp add --transport http github https://api.githubcopilot.com/mcp/ \
  --header "Authorization: Bearer YOUR_GITHUB_PAT"
```

**แทน `YOUR_GITHUB_PAT` ด้วย token จาก GitHub Settings → Developer settings → Personal access tokens**
...
### ผ่าน CLI
```bash
# HTTP Server (GitHub ต้องมี auth header)
claude mcp add --transport http github https://api.githubcopilot.com/mcp/ \
  --header "Authorization: Bearer YOUR_GITHUB_PAT"
...
# User scope (all projects, private)
...
```bash
# ต้องมี GitHub Personal Access Token (PAT)
claude mcp add --transport http github https://api.githubcopilot.com/mcp/ \
  --header "Authorization: Bearer YOUR_GITHUB_PAT"
```
**Use Cases:**
...
```bash
# Tool Search (สำหรับ custom MCP proxy)
...
# MCP server allowlist mode
export CLAUDE_CODE_MCP_ALLOWLIST_ENV=1      # ใช้ safe baseline environment สำหรับ MCP servers
```

> **MCP Timeout:** `MCP_TIMEOUT` ตั้ง startup timeout (เช่น `MCP_TIMEOUT=30000 claude`) | `MCP_TOOL_TIMEOUT` ตั้ง per-tool timeout (หรือใช้ `"timeout"` ใน `.mcp.json` สำหรับ server เฉพาะ)
...
| Server not starting | Path ผิด หรือ server crash | ตรวจสอบ path ใน config, ดู debug logs |
| Tools not appearing | Server not loaded | Check `claude mcp list` |
| Permission denied | Script not executable | `chmod +x script.sh` |
| Auth fails | Wrong token | Use `/mcp` for OAuth |
| Server startup timeout | Server ใช้เวลาเริ่มนานเกิน | ตั้ง `MCP_TIMEOUT=30000` (ms) เพื่อขยาย startup timeout |
| Tool execution timeout | Tool ใช้เวลาทำงานนาน | ตั้ง `MCP_TOOL_TIMEOUT=60000` หรือเพิ่ม `"timeout"` ใน `.mcp.json` |
...
| ใช้ `project` scope เมื่ออยากแชร์ config กับทีม (ผ่าน `.mcp.json`) หรือ `local` เมื่อต้องการ private | ตั้งทุกอย่างเป็น `user` (จะโหลดทุกโปรเจค) |
...
```bash
# Limit thinking tokens (saves context) — ใช้ได้เมื่อ disable adaptive thinking
export MAX_THINKING_TOKENS=10000  # Default: 32000

# Enable tool search — ค่าที่รองรับ:
#   true  = defer all tools (โหลดเมื่อต้องการ)
#   auto  = threshold mode (โหลดล่วงหน้าถ้า context < 10%)
#   false = load upfront (โหลดทุก tool ตั้งแต่เริ่ม)
export ENABLE_TOOL_SEARCH=auto

# ปรับ context สำหรับ auto-compaction
export CLAUDE_AUTOCOMPACT_PCT_OVERRIDE=80  # เปอร์เซ็นต์ที่จะ compact อัตโนมัติ
```

> **MCP Environment Variables:** `MAX_MCP_OUTPUT_TOKENS` เป็น env var ที่ official docs รองรับ — ใช้ตั้ง token limit สำหรับ MCP output (default: 25,000 tokens) ควบคู่กับ `MCP_TIMEOUT` (startup timeout) และ `MCP_TOOL_TIMEOUT` (per-tool timeout)
...
Enable: `/config` → เลือก "Editor mode" → "vim"

> **หมายเหตุ:** คำสั่ง `/vim` ถูกลบออกใน v2.1.92 ต้องใช้ `/config` แทน
...
# 2. เปิด Claude แล้วรัน /doctor ใน session
claude
# จากนั้นพิมพ์:
# > /doctor
...
# ขั้นที่ 3: ถ้า server ใช้เวลา startup นาน ขยาย timeout:
MCP_TIMEOUT=30000 claude   # ให้เวลา server startup 30 วินาที
```
  ...
  test-analysis:
    ...
    steps:
      ...
      - name: Run tests with Claude analysis
        run: |
          # รัน test และให้ Claude วิเคราะห์ (ผล = plain text)
      ...
      - name: Check coverage threshold
        run: |
          # แยก coverage จาก text output ด้วย grep
          COVERAGE=$(grep -oP '\d+(\.\d+)?(?=%)' analysis.txt | head -1 || echo "0")
...
/model                    # Change model
/plan                     # Plan mode
/config                   # Settings (รวม vim mode toggle)
/agents                   # Manage agents
/mcp                      # MCP servers
/plugin                   # Plugins
/batch                    # Parallel codebase operations
/rewind                   # Rewind to checkpoint
/btw                      # Side question (ไม่กิน context)
/diff                     # Interactive diff viewer
/fast                     # Toggle fast mode
/schedule                 # Manage recurring routines
...
```bash
# Authentication
ANTHROPIC_API_KEY                 # API key (สำหรับ API billing)
...
# Thinking
MAX_THINKING_TOKENS               # Fixed thinking budget (เมื่อ disable adaptive thinking)
CLAUDE_CODE_DISABLE_THINKING      # Force-disable thinking (set to 1)

# Features
ENABLE_TOOL_SEARCH                # MCP tool search via custom proxy
CLAUDE_CODE_DISABLE_BACKGROUND_TASKS  # Disable background agents (set to 1)
CLAUDE_CODE_DISABLE_AUTO_MEMORY   # Disable auto memory (set to 1)

# Debugging
DISABLE_AUTOUPDATER               # Disable auto-updates

# ดู env vars ทั้งหมดได้ที่: https://code.claude.com/docs/en/env-vars
```
...
- `/feedback` หรือ `/bug` command to report issues
